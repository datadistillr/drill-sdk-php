...
//		$this->cachedEnabledPlugins = $this->getEnabledStoragePlugins();
     ...
     * @return Table[]
     * @throws Exception
     * @deprecated v0.7.12 use getSheets()
     */
     ...
     * @throws Exception
     */
    public function getSheets(string $pluginName, string $filePath, string $pluginType): array
    {
        $this->logMessage(LogType::Request, 'Starting getSheets()');

        $tab = $pluginType === 'googlesheets' ? '.`tab[0]`' : '';

        $results = $this->query("SELECT _sheets AS sheets FROM `{$pluginName}`.{$filePath}{$tab} LIMIT 1")->getRows();
     ...
     * @throws Exception
     */
    public function getSheetColumns(string $pluginName, string $filePath, string $sheetName): array
    {
        $this->logMessage(LogType::Request, 'Starting getSheetColumns()');

        $columns = [];

        try {
            $sql = "SELECT * FROM `{$pluginName}`.{$filePath}.{$sheetName} LIMIT 1";
            $responseData = $this->query($sql, RequestFunction::MapQuery)->getSchema();
        ...
        $this->logMessage(LogType::Request, 'Ending getSheetColumns()');
        return $columns;
    }
            ...
            case PluginType::GoogleSheets:
                ...
                if ($itemCount < 1) {
                    ...
                    foreach ($list as $name) {
                        $title = $this->getGSTitle($pluginName, $name);
                    ...
                    $results = $this->getSheetColumns($pluginName, $filePath, $remaining);
                }
                break;
